...
if (isset($_GET['id'])) {
    ...
    try {
        // Atualizar o status para 'agendado' ou 'confirmado'
        $stmtUpdate = $pdo->prepare("UPDATE agendamentos SET status = 'agendado' WHERE id = :id AND status = 'pendente'");
        $stmtUpdate->execute([':id' => $id_agendamento]);

        if ($stmtUpdate->rowCount() > 0) {
            echo "<script>alert('Agendamento aprovado com sucesso!'); window.location.href='../painel.php?page=agendamentos';</script>";
        } else {
        ...
        die("Erro ao aprovar agendamento: " . $e->getMessage());
    }
} else {
